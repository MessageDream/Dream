...
func (self *RAdvantagesHandler) Get() {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-advantages-del/([0-9]+)$"):
		//删除GET状态 删除內容
		tid, _ := self.GetInt(":tid")

		if e := bll.DelTopic(tid); e != nil {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-advantages-edit/([0-9]+)$"):
		//编辑GET状态
		self.Data["asidepage"] = "root_advantages_edit"
		tid, _ := self.GetInt(":tid")
		self.Data["topic"] = bll.GetTopic(tid)
	...
	case self.Ctx.Request.RequestURI == "/root-advantages-list":
		...
		self.Data["topics"] = bll.GetAllTopicByCid(cid, 0, 0, 0, "asc")
		...
		case common.Rex(self.Ctx.Request.RequestURI, "^/root-advantages-edit/([0-9]+)$"):
			...
			if title != "" && content != "" && tid != 0 {
				...
				if self.Data["file_location"] != "" {
					//保存编辑
					if e := bll.SetTopic(tid, cid, nodeid, uid, 0, title, content, uname, self.Data["file_location"].(string)); e != nil {
		...
		case self.Ctx.Request.RequestURI == "/root-advantages":
			...
			if handler != nil && title != "" && content != "" {
				...
				if err != nil {
					...
					output_align := "center"
					common.Thumbnail(input_file, output_file, output_size, output_align, "white")
					if e := bll.SetTopic(0, cid, nodeid, uid, 0, title, content, uname, path); e != nil {
