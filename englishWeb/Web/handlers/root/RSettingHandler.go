...
func (self *RSettingHandler) Get() {
	...
	switch {
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-setting-setroot-del/([0-9]+)$"):
		rootid, _ := self.GetInt(":rid")
		if e := bll.DelUser(rootid); e != nil {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-setting-setroot-edit/([0-9]+)$"):
		self.Data["asidepage"] = "root_setting_setroot_edit"
		rootid, _ := self.GetInt(":rid")
		self.Data["root"] = bll.GetUser(rootid)
		self.TplNames = "root/setting_setroot.html"

	case self.Ctx.Request.RequestURI == "/root-setting-setroot":
		self.Data["asidepage"] = "root_setting_setroot"
		self.Data["roots"] = bll.GetAllUserByRole(-1000)
		self.TplNames = "root/setting_setroot.html"
...
func (self *RSettingHandler) Post() {
	...
	case self.Ctx.Request.RequestURI == "/root-setting-setroot":
		...
		if newroot != "" && realname != "" && curpassword != "" && repassword != "" && newpassword == repassword {
			...
			usr := bll.GetUserByNickname(sess_username)

			if common.Validate_password(usr.Password, curpassword) {

				if e := bll.AddUser("", newroot, realname, common.Encrypt_password(newpassword, nil), -1000); e != nil {
	...
	case self.Ctx.Request.RequestURI == "/root-setting-password":
		...
		if oldpassword != "" && repassword != "" && newpassword == repassword {
			...
			if common.Validate_password(usr.Password, oldpassword) {
				usr.Password = common.Encrypt_password(newpassword, nil)
				if e := bll.SaveUser(usr); e != nil {
	...
	case self.Ctx.Request.RequestURI == "/root-setting":
		...
		bll.SetKV("title", title)
		bll.SetKV("title_en", title_en)
		bll.SetKV("keywords", keywords)
		bll.SetKV("description", description)

		bll.SetKV("company", company)
		bll.SetKV("copyright", copyright)
		bll.SetKV("site_email", site_email)

		bll.SetKV("tweibo", tweibo)
		bll.SetKV("sweibo", sweibo)

		bll.SetKV("statistics", statistics)
