...
func (self *RContactHandler) Post() {
	...
	if self.Ctx.Request.RequestURI == "/root-contact-new-node" {
		...
		//新建内容分类
		if title != "" {
			if e := bll.AddNode(title, content, cid, 1); e != nil {
		...
		if msg == "" {
			switch {
			case common.Rex(self.Ctx.Request.RequestURI, "^/root-contact-edit/([0-9]+)$"):
				//编辑POST状态
				...
				if nodeid != 0 && title != "" && content != "" && tid != 0 {
					...
					if self.Data["file_location"] != "" {
						//保存编辑
						if e := bll.SetTopic(tid, cid, nodeid, uid, 0, title, content, sess_username, self.Data["file_location"].(string)); e != nil {
			...
			case self.Ctx.Request.RequestURI == "/root-contact":
				...
				case /* handler != nil &&*/ nodeid != 0 && title != "" && content != "":
					...
					if e := bll.SetTopic(0, cid, nodeid, uid, 0, title, content, username, path); e != nil {
