...
func (self *RSetvideoHandler) Get() {
	...
	var tid int64 = 0
	for _, v := range bll.GetAllTopicByCid(1, 0, 1, 30, "id") {
		if v.Id > 0 {
			tid = v.Id
		}
	}
	...
	//self.Data["nodes"] = bll.GetAllNodeByCid(1, 0, 0, "id")
...
func (self *RSetvideoHandler) Post() {
	...
	if msg == "" {
		...
		if handler == nil {
			...
			if m := bll.GetTopic(tid); m.Attachment != "" {
				path = m.Attachment
			} else {
		...
		if self.Data["MsgErr"] == "" && path != "" && ftitle != "" && content != "" {
			...
			if e := common.Thumbnail(input_file, output_file, output_size, output_align, "white"); e != nil {
				fmt.Println("Thumbnail", e)
			}

			if e := common.Watermark(watermark_file, input_file, output_file, output_align); e != nil {
				fmt.Println("Watermark", e)
			}
			if e := bll.SetTopic(2, cid, nodeid, uid, 30, ftitle, content, "root", path); e != nil {
