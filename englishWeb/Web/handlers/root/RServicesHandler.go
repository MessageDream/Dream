...
func (self *RServicesHandler) Get() {
	...
	self.Data["topics"] = bll.GetAllTopicByCid(cid, 0, 0, 0, "id")
	switch {
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-services-del/([0-9]+)$"):
...
func (self *RServicesHandler) Post() {
	...
	if msg == "" {
		switch {
		case common.Rex(self.Ctx.Request.RequestURI, "^/root-services-edit/([0-9]+)$"):
			//编辑POST状态
			...
			if nodeid != 0 && title != "" && content != "" && tid != 0 {
				if handler != nil {
					...
					if err != nil {
						...
						if self.Data["file_location"] != nil {
							if common.Exist("." + self.Data["file_location"].(string)) {
				...
				if self.Data["file_location"] != "" {
					//保存编辑
					if e := bll.SetTopic(tid, cid, nodeid, uid, 1, title, content, sess_username, self.Data["file_location"].(string)); e != nil {
		...
		case self.Ctx.Request.RequestURI == "/root-services":
			...
			case handler != nil && nodeid != 0 && title != "" && content != "":
				...
				if err != nil {
					...
					output_align := "center"
					common.Thumbnail(input_file, output_file, output_size, output_align, "white")
					if e := bll.SetTopic(0, cid, nodeid, uid, 1, title, content, sess_username, path); e != nil {
		...
		case self.Ctx.Request.RequestURI == "/root-services-new-node":
			...
			content := self.GetString("content")
			if title != "" {
				bll.AddNode(title, content, 3, -1000)
