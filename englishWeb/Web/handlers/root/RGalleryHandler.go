...
func (self *RGalleryHandler) Get() {
	...
	self.Data["images"] = bll.GetAllFileByCtype(10)
	self.Data["images2"] = bll.GetAllFileByCtype(20)
	self.Render()
...
func (self *RGalleryHandler) Post() {
	...
	if handler != nil {
		...
		if err != nil {
			...
			io.Copy(f, file)
			bll.AddFile(10, path, "") //ctype設置為10,以免跟正常的已標記上傳文件混淆.
	...
	if handler2 != nil {
		...
		if err != nil {
			...
			io.Copy(f, file2)
			bll.AddFile(20, path, "") //ctype設置為20,以免跟正常的已標記上傳文件混淆.
