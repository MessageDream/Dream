...
func (self *RApi) Get() {
	...
	switch {
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-about-gallery-edit/([0-9]+)$"):
		//# Gallery编辑GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			...
			img := bll.GetFile(editmid)
			self.Data["img"] = img
			self.TplNames = "root/gallery_editurl.html"
			self.Render()
		}

	case common.Rex(self.Ctx.Request.RequestURI, "^/root-about-gallery-del/([0-9]+)$"):
		//# Gallery删除GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
			if mid, _ := self.GetInt(":delmid"); mid != 0 {
				if e := bll.DelFile(mid); e != nil {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-about-node-del/([0-9]+)$"):
		//#ABOUT 节点删除 GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
			if mid, _ := self.GetInt(":nid"); mid != 0 {
				if e := bll.DelNode(mid); e != nil {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-contact-node-del/([0-9]+)$"):
		//#contact 节点删除 GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-about-topic-del/([0-9]+)$"):
		//#ABOUT 内容删除GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
			if mid, _ := self.GetInt(":tid"); mid != 0 {
				if e := bll.DelTopic(mid); e != nil {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-crafts-topic-del/([0-9]+)$"):
		// crafts 内容删除GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-services-topic-del/([0-9]+)$"):
		// services 内容删除GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Redirect("/root-login", 302)
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-about-topic-edit/([0-9]+)$"):
		//ABOUT 内容编辑GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Redirect("/root-login", 302)
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-services-topic-edit/([0-9]+)$"):
		// services 内容编辑 GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Redirect("/root-login", 302)
		} else {
			...
			self.Data["nodes"] = bll.GetAllNodeByCid(cid, 0, 0, 0, "id")
			self.TplNames = "root/published.html"
			self.Render()
		}
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-contact-topic-edit/([0-9]+)$"):
		//CONTACT 内容编辑GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Redirect("/root-login", 302)
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-crafts-topic-edit/([0-9]+)$"):
		//crafts 内容编辑GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Redirect("/root-login", 302)
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-information-topic-del/([0-9]+)$"):
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-contact-topic-del/([0-9]+)$"):
		//contact 内容删除GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-about-node-edit/([0-9]+)$"):
		// about 节点编辑GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
			...
			self.Data["node"] = bll.GetNode(nid)
			self.TplNames = "root/published_node.html"
			self.Render()
		}

	case common.Rex(self.Ctx.Request.RequestURI, "^/root-contact-node-edit/([0-9]+)$"):
		//contact 节点编辑GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-services-node-edit/([0-9]+)$"):
		//services 节点编辑GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-information-node-edit/([0-9]+)$"):
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-crafts-node-edit/([0-9]+)$"):
		//crafts节点编辑GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-information-topic-edit/([0-9]+)$"):
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-services-node-del/([0-9]+)$"):
		//services 节点删除 GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-information-node-del/([0-9]+)$"):
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-crafts-node-del/([0-9]+)$"):
		//#crafts 删除节点 GET状态
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			self.Ctx.Redirect(302, "/root-login")
		} else {
...
func (self *RApi) Post() {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-about-gallery-edit/([0-9]+)$"):
		...
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			...
			if e := bll.SetFile(editmid, img.Pid, img.Ctype, img.Filename, img.Content, img.Hash, img.Location, url, img.Size); e != nil {
		...
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			...
			if handler != nil && e == nil {
				...
				if err != nil {
					...
					output_size := "232x135"
					output_align := "center"
					common.Thumbnail(input_file, output_file, output_size, output_align, "white")

					//若文件存在则删除，不存在就当忽略处理
					if file_location != "" {
						if common.Exist("." + file_location) {
			...
			if ftitle != "" && nid != 0 && content != "" {
				//保存编辑
				if e := bll.SetTopic(tid, cid, nid, uid, 1, ftitle, content, stitle, file_location); e != nil {
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-about-node-edit/([0-9]+)$"):
		...
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			...
			//编辑NODE
			if title != "" && nid != 0 {
				if e := bll.SetNode(nid, title, content, cid, 1); e != nil {
		...
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			...
			if handler != nil && e == nil {
				...
				if err != nil {
					...
					output_size := "534"
					output_align := "center"
					common.Thumbnail(input_file, output_file, output_size, output_align, "black")
			...
			if ftitle != "" && nid != 0 && content != "" {
				//保存编辑
				if e := bll.SetTopic(tid, cid, nid, uid, 0, ftitle, content, stitle, file_location); e != nil {
					self.Data["MsgErr"] = "你提交的修改保存失败，无法写入数据库！"
	...
	case common.Rex(self.Ctx.Request.RequestURI, "^/root-contact-topic-edit/([0-9]+)$"):
		...
		if sess_role, _ := self.GetSession("userrole").(int64); sess_role != -1000 {
			...
			if ftitle != "" && nid != 0 && content != "" {
				//保存编辑
				if e := bll.SetTopic(tid, cid, nid, uid, 0, ftitle, content, "", file_location); e != nil {
					self.Data["MsgErr"] = "你提交的修改保存失败，无法写入数据库！"
