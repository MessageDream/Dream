...
func (self *RAboutHandler) Get() {
	...
	self.Data["topics"] = bll.GetAllTopicByCid(1, 0, 0, 1, "id")
	self.Data["nodes"] = bll.GetAllNodeByCid(1, 0, 0, 0, "id")
...
func (self *RAboutHandler) Post() {
	...
	if msg == "" {
		switch {
		case common.Rex(self.Ctx.Request.RequestURI, "^/root-about-edit/([0-9]+)$"):
			//编辑POST状态
			tid, _ := self.GetInt(":tid")
			...
			if nodeid != 0 && ftitle != "" && content != "" && tid != 0 {
				...
				if self.Data["file_location"] != "" {
					if e := bll.SetTopic(tid, cid, nodeid, uid, 1, ftitle, content, stitle, self.Data["file_location"].(string)); e != nil {
		...
		case self.Ctx.Request.RequestURI == "/root-about":
			...
			case handler != nil && nodeid != 0 && ftitle != "" && content != "":
				...
				if err != nil {
					...
					output_align := "center"
					common.Thumbnail(input_file, output_file, output_size, output_align, "black")
					if e := bll.SetTopic(0, cid, nodeid, uid, 1, ftitle, content, stitle, path); e != nil {
		...
		case self.Ctx.Request.RequestURI == "/root-about-new-node":
			...
			content := self.GetString("content")
			if title != "" {
				bll.AddNode(title, content, cid, -1000)
