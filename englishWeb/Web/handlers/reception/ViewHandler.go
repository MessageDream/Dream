...
func (self *ViewHandler) Get() {
	...
	if tid_handler.Id > 0 {
		...
		bll.UpdateTopic(tid, tid_handler)

		self.Data["article"] = tid_handler
		self.Data["replys"] = bll.GetReplyByPid(tid, 0, 0, "id")

		tps := bll.GetAllTopicByCid(tid_handler.Cid, 0, 0, 0, "asc")
		...
		if sess_userrole, _ := self.GetSession("userrole").(int64); sess_userrole == -1000 {
			...
			common.Writefile("./archives/"+tid_path, tid_name, rs)
